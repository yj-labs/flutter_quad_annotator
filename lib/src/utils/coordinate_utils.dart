...
class CoordinateUtils {
  ...
  /// [imageInfo] 图片信息
  /// 返回图片坐标系中的点
  static Point<double> convertScreenToImageCoordinates(
    Point<double> screenPoint,
    QuadImageInfo imageInfo,
  ) {
    // 减去图片在容器中的偏移量
    final adjustedPoint = screenPoint.subtractOffset(imageInfo.offset);

    // 计算在显示图片中的相对位置（0-1）
    final relativeX = (adjustedPoint.x / imageInfo.displaySize.width).clamp(
      0.0,
      1.0,
    );
    final relativeY = (adjustedPoint.y / imageInfo.displaySize.height).clamp(
      0.0,
      1.0,
    );
  ...
  /// [imageInfo] 图片信息
  /// 返回图片真实坐标列表
  static List<Point<double>> convertToImageCoordinates(
  ...
  /// [imageInfo] 图片信息
  /// 返回视图坐标列表
  static List<Point<double>> convertToViewCoordinates(
  ...
  /// [imageInfo] 图片信息
  /// 返回限制后的坐标
  static Point<double> clampToImageBounds(
