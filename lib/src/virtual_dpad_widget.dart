...
class CircularDPadPainter extends CustomPainter {
  ...
  void _drawSectorButton(Canvas canvas, Offset center, double radius, double innerRadius, int direction) {
  ...
  /// 绘制方向图标
  void _drawDirectionIcon(Canvas canvas, Offset center, double radius, double innerRadius, int direction) {
...
class _VirtualDPadWidgetState extends State<VirtualDPadWidget> {
  ...
  void _initializePosition() {
    ...
    final availableWidth = widget.screenSize.width - totalSize.width - margin * 2;
    final availableHeight = widget.screenSize.height - totalSize.height - margin * 2;
  ...
  int? _getDirectionFromPosition(Offset localPosition) {
    ...
    final radius = (widget.config.centerButtonSize + widget.config.size * 2) / 2;
  ...
  void _startContinuousMove(int direction) {
    _continuousMoveTimer = Timer.periodic(const Duration(milliseconds: 100), (timer) {
    ...
    return Positioned(
      ...
      child: Row(
          ...
          children: [
            ...
            GestureDetector(
              onTapDown: (details) {
                if (_isCenterButtonTap(details.localPosition) && !_isDraggingPanel) {
                  // 只有在非拖拽状态下才允许切换顶点
                  final nextIndex = (widget.selectedVertexIndex + 1) % widget.totalVertices;
              ...
              onPanStart: (details) {
                // 如果不是方向键区域，则处理拖拽
                if (_isCenterButtonTap(details.localPosition) || _getDirectionFromPosition(details.localPosition) == null) {
